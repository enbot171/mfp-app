...
// Master cells may hold amounts as text with thousands separators ("1,000") —
// strip commas/$/spaces before parsing so 1,000 reads as 1000, not 1.
...
export function matchPayments(paymentRows, masterRows, masterHeaders, processedFingerprints = []) {
  ...
  // Exact duplicates within this same upload (e.g. the same transaction copied
  // across multiple bank tabs) — only the first occurrence is processed.
  ...
  return paymentRows.map((payRow) => {
    // ── Duplicate (already pushed, or repeated earlier in this upload) ─────────
    if (processed.has(payRow.fingerprint) || seenInFile.has(payRow.fingerprint)) {
      return {
        ...payRow,
        matchType:      "duplicate",
        isDuplicate:    true,
    ...
    seenInFile.add(payRow.fingerprint); // first occurrence — remember it
    ...
    // ── Amounts ───────────────────────────────────────────────────────────────
    const pledgeAmount = masterRow ? num(masterRow[MASTER_COLS.PLEDGE_AMOUNT]) : 0;
    let currentAmount  = 0;
    let newAmount      = payRow.amount;

    if (masterRow && monthColIndex !== -1) {
      const cellKey     = `${masterRowIndex},${monthColIndex}`;
      const sheetValue  = num(masterRow[monthColIndex]);
