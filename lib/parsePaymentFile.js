...
// Free-text bank descriptions can be long and contain other 3-digit+letter
// sequences (invoice/ref numbers) after the payer's note. The MF number is
// almost always written at the very start, so only scan the first 25 chars to
// avoid picking up a wrong MF from deeper in the description.
const DESC_SCAN_LEN = 25;
const extractMFFromText = (text) => extractMF(String(text ?? "").slice(0, DESC_SCAN_LEN));
...
function makeRow(idx, sheetName, mfResult, amount, date, rawRef, description = "") {
  ...
  return {
    ...
    date,
    month,
    // Raw payer note / name from the bank file — shown on unmatched rows for cross-checking
...
function parseFormat1(rows, sheetName, startIdx) {
  return rows.slice(1)
    ...
    .map((row, i) => {
      ...
      return makeRow(startIdx + i, sheetName, mfResult, deposit, date, rawRef, remarks);
    });
}
...
function parseUobBo(rows, sheetName, startIdx) {
  return rows.slice(1)
    ...
    .map((row, i) => {
      ...
      return makeRow(startIdx + i, sheetName, mfResult, amount, date, rawRef, name);
    });
}
...
function parseDbsPn(rows, sheetName, startIdx) {
  return rows.slice(2)
    ...
    .map((row, i) => {
      ...
      return makeRow(startIdx + i, sheetName, mfResult, credit, date, rawRef, desc2);
    });
}
...
function parseBank5(rows, sheetName, startIdx) {
  return rows.slice(1)
    ...
    .map((row, i) => {
      ...
      return makeRow(startIdx + i, sheetName, mfResult, credit, date, rawRef, desc);
    });
}
