...
// Handles YYYYMMDD stored as integer (e.g. 20260519) or float (20260519.0)
...
function normaliseMFFromRaw(raw) {
  ...
  return { value: `MF${stripped.toUpperCase()}`, corrected: true };
}
...
function parseFormat1(rows, sheetName, startIdx) {
  return rows.slice(1)
    ...
    .map((row, i) => {
      ...
      const date          = excelSerialToDate(typeof dateSerial === "number" ? dateSerial : null);
...
function parseUobBo(rows, sheetName, startIdx) {
  return rows.slice(1)
    ...
    .map((row, i) => {
      ...
      const dateVal = row[5];
      const date    = excelSerialToDate(typeof dateVal === "number" ? dateVal : null);
...
function parseDbsBo(rows, sheetName, startIdx) {
  return rows.slice(1)
    ...
    .map((row, i) => {
      ...
      const amount  = parseFloat(row[2]) || 0;
      const dateVal = row[3];
      const date    = yyyymmddToDate(dateVal);
      const mfResult = normaliseMFFromRaw(mfRaw);
      const rawRef  = [mfRaw.toUpperCase(), String(dateVal), String(amount)].join("|");
      return makeRow(startIdx + i, sheetName, mfResult, amount, date, rawRef);
    });
