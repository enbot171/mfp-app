...
// For columns where MF is a direct value: col[0] of UOB BO, DBS BO
function normaliseMFFromRaw(raw) {
  const s = (raw ?? "").trim();
  if (!s) return { value: "", corrected: false };
  const stripped = s.replace(/^MFP\s*/i, "").trim();
  // "MF" alone with no digits = invalid
  if (!stripped || /^MF$/i.test(stripped)) return { value: "", corrected: false };
  // Reject junk that isn't MF-shaped (e.g. a "9 Mar - 15 Mar" date-range separator
  // row landing in the MF column) — an MF has no spaces.
  if (/\s/.test(stripped)) return { value: "", corrected: false };
  if (/^MF/i.test(stripped)) return { value: stripped.toUpperCase(), corrected: false };
  // Bare value: only prepend MF if it starts with digits (a partial NRIC)
  if (/^\d/.test(stripped)) return { value: `MF${stripped.toUpperCase()}`, corrected: true };
  return { value: "", corrected: false };
}

// For free-text fields: remarks, description columns
function extractMFFromText(text) {
  const s = (text ?? "").trim();
  // Explicit MF + digits + at least one letter
  let m = s.match(/\bMF(\d+[A-Z][A-Z0-9]*)\b/i);
  if (m) return { value: `MF${m[1].toUpperCase()}`, corrected: false };
  // MFP prefix
  m = s.match(/\bMFP\s*(\d+[A-Z][A-Z0-9]*)\b/i);
  if (m) return { value: `MF${m[1].toUpperCase()}`, corrected: true };
  // Bare partial NRIC: 3-4 digits + 1+ letters at word boundary
  m = s.match(/\b(\d{3,4}[A-Z][A-Z0-9]*)\b/i);
  if (m) return { value: `MF${m[1].toUpperCase()}`, corrected: true };
  return { value: "", corrected: false };
}
